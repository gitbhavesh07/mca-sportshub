...
test('check onchange', async() => {
    ...
    await waitFor(() => {
        expect(toastSuccesSpy).toHaveBeenCalledWith('Player Transferred Successfully!',
        {
         position: "top-right",
         autoClose: 5000,
...
test('transfer button', async () => {
    ...
    await waitFor(() => {
        expect(toastErrorSpy).toHaveBeenCalledWith('Failed to transfer players!',
        {
         position: "top-right",
         autoClose: 5000,
         hideProgressBar: false,
